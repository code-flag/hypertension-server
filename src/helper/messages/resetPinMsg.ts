...
export const resetPinMsg = (data: any) => {
  let resetUrl;

  if (data.userType === "user") {
    resetUrl = `https://user.psi.org/reset-pin-account/${data.token}`;
  } else if (data.userType === "staff" || data.userType === "admin") {
    resetUrl = `https://staff.psi.org/reset-pin-account/${data.token}`;
  } else if (data.userType === "organization") {
    resetUrl = `https://organization.psi.org/reset-pin-account/${data.token}`;
  }
          ...
          <tbody>
              ...
              <tr >
                  <td align="left" style="border: none;
                      ...
                      <br><br>
                      <a href="${resetUrl}" style="background-color: #026395; color: #fff; padding: 10px 20px; cursor: pointer; text-decoration: none">Confirm Email</a>
